...
- Un journal d'utilisation (`sound_usage_log.json`) conserve un compteur d'utilisation par fichier son, à des fins statistiques uniquement.
- Le tirage au sort se fait de manière uniforme parmi tous les sons disponibles, quel que soit leur usage passé.
...
- Le tirage est uniforme sur l'ensemble des sons disponibles : le compteur d'utilisation du journal persistant ne sert qu'à des fins statistiques et n'influence pas la sélection.
- Ce compteur n'est pas mis à jour pendant l'attribution des sons ; sa reconstruction reste déclenchée manuellement.
...
- Elle recherche récursivement les fichiers `<n...>_metadata.json` dans un dossier racine (par défaut `videostorage`).
- La clé `audio` de chaque fichier metadata, lorsqu'elle est présente, compte pour une utilisation du son correspondant.
